...
AllanVarianceComputor::AllanVarianceComputor(ros::NodeHandle& nh, std::string config_file, std::string output_path)
  ...
  // for now assumes same sampling properties as IMU
  calculate_mag_ = get(node, "mag_topic", mag_topic);
  if (calculate_mag_) {
    input_topics_.push_back(mag_topic);
  }
...
void AllanVarianceComputor::run(std::string bag_path) {
  ...
  try {
    ...
    for (const rosbag::MessageInstance& msg : view) {
      ...
      if (msg.isType<sensor_msgs::Imu>()) {
        ...
        if (tCurrNanoSeconds_ < lastImuTime_) {
          skipped_imu_++;
          ROS_ERROR_STREAM("IMU out of order. Current(ns): " << tCurrNanoSeconds_ - firstTime_
                                                             << " Last(ns): " << lastImuTime_ - firstTime_ << " ("
                                                             << skipped_imu_ << " dropped)");
          continue;
        }
        lastImuTime_ = tCurrNanoSeconds_;
    ...
    if (calculate_mag_) {
      start = clock();
      int mag_counter = 0;
      bool first_mag_msg = true;
      uint64_t first_mag_time = 0;
      uint64_t last_mag_time = 0;
      uint64_t skipped_mag = 0;

      for (const rosbag::MessageInstance& msg : view) {
        // Fill IMU buffer
        if (msg.isType<sensor_msgs::MagneticField>()) {
          sensor_msgs::MagneticFieldConstPtr magnetic_msg = msg.instantiate<sensor_msgs::MagneticField>();
          uint64_t tCurrNanoSeconds = magnetic_msg->header.stamp.toNSec();

          mag_counter++;

          // Subsample IMU measurements
          if (mag_counter % imu_skip_ != 0 || mag_counter / imu_rate_ > sequence_time_) {
            continue;
          }

          if (difftime(clock(), start) / CLOCKS_PER_SEC >= 2.0) {
            ROS_INFO_STREAM(mag_counter / imu_rate_ << " / " << sequence_time_ << " seconds loaded");
            start = clock();
          }

          if (first_mag_msg) {
            first_mag_msg = false;
            first_mag_time = tCurrNanoSeconds;
            last_mag_time = tCurrNanoSeconds;
          }

          if (tCurrNanoSeconds < last_mag_time) {
            skipped_mag++;
            ROS_ERROR_STREAM("IMU out of order. Current(ns): " << tCurrNanoSeconds - first_mag_time
                                                               << " Last(ns): " << last_mag_time - first_mag_time
                                                               << " (" << skipped_mag << " dropped)");
            continue;
          }
          last_mag_time = tCurrNanoSeconds;

          MagneticMeasurement input;
          input.t = magnetic_msg->header.stamp.toNSec();
          input.I_m_WI = Eigen::Vector3d(
              magnetic_msg->magnetic_field.x, magnetic_msg->magnetic_field.y, magnetic_msg->magnetic_field.z);

          mag_buffer_.push_back(input);
        }
        if (!nh_.ok()) {
          ROS_ERROR_STREAM("Stop requested, closing the bag!");
          bag.close();
...
void AllanVarianceComputor::allanVariance() {
  ...
  std::map<int, std::vector<std::vector<double>>> mag_averages_map;
...
#pragma omp parallel for
  for (int period = period_min; period < period_max; period++) {
    ...
    current_average = {0.0, 0.0, 0.0};
    std::vector<std::vector<double>> mag_averages;

    // Compute Averages
    for (int j = 0; j < ((int)mag_buffer_.size() - max_bin_size); j += (max_bin_size - overlap)) {
      // get average for current bin
      for (int m = 0; m < max_bin_size; m++) {
        // Magnetic Field
        current_average[0] += mag_buffer_[j + m].I_m_WI[0];
        current_average[1] += mag_buffer_[j + m].I_m_WI[1];
        current_average[2] += mag_buffer_[j + m].I_m_WI[2];
      }
      ...
      mag_averages.push_back(current_average);
      current_average = {0.0, 0.0, 0.0};
    }

    {
      ...
      num_averages = mag_averages.size();
      ROS_INFO_STREAM("Computed " << num_averages << " magnetometer averages for period " << period_time << " ("
                                  << (10000 - mag_averages_map.size()) << " left)");
      mag_averages_map.insert({period, mag_averages});
    }
  }
  ...
  for (int period = period_min; period < period_max; period++) {
    ...
    std::vector<double> avar = {allan_variance[0] / (2 * (num_averages - 1)),
    ...
    std::vector<double> allan_deviation = {std::sqrt(avar[0]),
                                           std::sqrt(avar[1]),
                                           std::sqrt(avar[2]),
                                           std::sqrt(avar[3]),
                                           std::sqrt(avar[4]),
    ...
    std::vector<double> magnetic_allan_deviation;
    if (calculate_mag_) {
      std::vector<std::vector<double>> mag_averages = mag_averages_map.at(period);
      num_averages = mag_averages.size();
      ROS_INFO_STREAM("Computed " << num_averages << " magnetometer bins for sampling period " << period_time
                                  << " out of " << mag_buffer_.size() << " measurements.");
      std::vector<double> mag_allan_variance = {0.0, 0.0, 0.0};
      for (int k = 0; k < num_averages - 1; k++) {
        mag_allan_variance[0] += std::pow(mag_averages[k + 1][0] - mag_averages[k][0], 2);
        mag_allan_variance[1] += std::pow(mag_averages[k + 1][1] - mag_averages[k][1], 2);
        mag_allan_variance[2] += std::pow(mag_averages[k + 1][2] - mag_averages[k][2], 2);
      }

      std::vector<double> avar = {
          mag_allan_variance[0] / (2 * (num_averages - 1)),
          mag_allan_variance[1] / (2 * (num_averages - 1)),
          mag_allan_variance[2] / (2 * (num_averages - 1)),
      };
      magnetic_allan_deviation = {std::sqrt(avar[0]), std::sqrt(avar[1]), std::sqrt(avar[2])};
    }

    writeAllanDeviation(allan_deviation, period_time, magnetic_allan_deviation);
  }
}

void AllanVarianceComputor::writeAllanDeviation(std::vector<double> deviation,
                                                double period,
                                                std::vector<double> magnetic_deviation) {
  ...
  if (!magnetic_deviation.empty()) {
    aVRecorder_.mag_x = magnetic_deviation[0];
    aVRecorder_.mag_y = magnetic_deviation[1];
    aVRecorder_.mag_z = magnetic_deviation[2];
  }
