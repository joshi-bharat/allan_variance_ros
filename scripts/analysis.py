...
magnetic_field = np.empty((0, 3), float)
...
with open(args.data) as input_file:
    ...
    for row in csv_reader:
        ...
        acceleration = np.append(
            acceleration,
            np.array([float(row[1]), float(row[2]), float(row[3])]).reshape(1, 3),
            axis=0,
        )
        rotation_rate = np.append(
            rotation_rate,
            np.array([float(row[4]), float(row[5]), float(row[6])]).reshape(1, 3),
            axis=0,
        )

        if len(row) > 7:
            magnetic_field = np.append(
                magnetic_field,
                np.array([float(row[7]), float(row[8]), float(row[9])]).reshape(1, 3),
                axis=0,
            )
...
accel_wn_intercept_x, xfit_wn = get_intercept(
    period[0:white_noise_break_point],
    acceleration[0:white_noise_break_point, 0],
    -0.5,
    1.0,
)
accel_wn_intercept_y, yfit_wn = get_intercept(
    period[0:white_noise_break_point],
    acceleration[0:white_noise_break_point, 1],
    -0.5,
    1.0,
)
accel_wn_intercept_z, zfit_wn = get_intercept(
    period[0:white_noise_break_point],
    acceleration[0:white_noise_break_point, 2],
    -0.5,
    1.0,
)
...
print("ACCELEROMETER:")
print(
    f"X Velocity Random Walk: {accel_wn_intercept_x: .5f} m/s/sqrt(s) {accel_wn_intercept_x*60: .5f} m/s/sqrt(hr)"
)
print(
    f"Y Velocity Random Walk: {accel_wn_intercept_y: .5f} m/s/sqrt(s) {accel_wn_intercept_y*60: .5f} m/s/sqrt(hr)"
)
print(
    f"Z Velocity Random Walk: {accel_wn_intercept_z: .5f} m/s/sqrt(s) {accel_wn_intercept_z*60: .5f} m/s/sqrt(hr)"
)

print(
    f"X Bias Instability: {accel_min_x: .5f} m/s^2 {accel_min_x*3600*3600: .5f} m/hr^2"
)
print(
    f"Y Bias Instability: {accel_min_y: .5f} m/s^2 {accel_min_y*3600*3600: .5f} m/hr^2"
)
print(
    f"Z Bias Instability: {accel_min_z: .5f} m/s^2 {accel_min_z*3600*3600: .5f} m/hr^2"
)
...
average_acc_white_noise = (
    accel_wn_intercept_x + accel_wn_intercept_y + accel_wn_intercept_z
) / 3
average_acc_bias_instability = (accel_min_x + accel_min_y + accel_min_z) / 3
average_acc_random_walk = (
    accel_rr_intercept_x + accel_rr_intercept_y + accel_rr_intercept_z
) / 3

# use worst value
worst_accel_white_noise = np.amax(
    [accel_wn_intercept_x, accel_wn_intercept_y, accel_wn_intercept_z]
)
worst_accel_random_walk = np.amax(
    [accel_rr_intercept_x, accel_rr_intercept_y, accel_rr_intercept_z]
)
...
fitted_model = generate_prediction(
    period,
    q_white=average_acc_white_noise,
    q_bias_instability=average_acc_bias_instability,
    q_walk=average_acc_random_walk,
)
...
gyro_wn_intercept_x, xfit_wn = get_intercept(
    period[0:white_noise_break_point],
    rotation_rate[0:white_noise_break_point, 0],
    -0.5,
    1.0,
)
gyro_wn_intercept_y, yfit_wn = get_intercept(
    period[0:white_noise_break_point],
    rotation_rate[0:white_noise_break_point, 1],
    -0.5,
    1.0,
)
gyro_wn_intercept_z, zfit_wn = get_intercept(
    period[0:white_noise_break_point],
    rotation_rate[0:white_noise_break_point, 2],
    -0.5,
    1.0,
)
...
print("GYROSCOPE:")
print(
    f"X Angle Random Walk: {gyro_wn_intercept_x: .5f} deg/sqrt(s) {gyro_wn_intercept_x * 60: .5f} deg/sqrt(hr)"
)
print(
    f"Y Angle Random Walk: {gyro_wn_intercept_y: .5f} deg/sqrt(s) {gyro_wn_intercept_y * 60: .5f} deg/sqrt(hr)"
)
print(
    f"Z Angle Random Walk: {gyro_wn_intercept_z: .5f} deg/sqrt(s) {gyro_wn_intercept_z * 60: .5f} deg/sqrt(hr)"
)
...
average_gyro_white_noise = (
    gyro_wn_intercept_x + gyro_wn_intercept_y + gyro_wn_intercept_z
) / 3
average_gyro_bias_instability = (gyro_min_x + gyro_min_y + gyro_min_z) / 3
average_gyro_random_walk = (
    gyro_rr_intercept_x + gyro_rr_intercept_y + gyro_rr_intercept_z
) / 3

# use worst value
worst_gyro_white_noise = np.amax(
    [gyro_wn_intercept_x, gyro_wn_intercept_y, gyro_wn_intercept_z]
)
worst_gyro_random_walk = np.amax(
    [gyro_rr_intercept_x, gyro_rr_intercept_y, gyro_rr_intercept_z]
)
...
yaml_file.write(
    "gyroscope_noise_density: " + repr(worst_gyro_white_noise * np.pi / 180) + " \n"
)
yaml_file.write(
    "gyroscope_random_walk: " + repr(worst_gyro_random_walk * np.pi / 180) + " \n"
)
yaml_file.write("\n")
...
if args.config:
    ...
    yaml_file.write(
        "update_rate: " + repr(update_rate) + " #Make sure this is correct\n"
    )
yaml_file.write("\n")
yaml_file.close()
...
fitted_model = generate_prediction(
    period,
    q_white=average_gyro_white_noise,
    q_bias_instability=average_gyro_bias_instability,
    q_walk=average_gyro_random_walk,
)
plt.loglog(period, fitted_model, "-k", label="fitted model")
